// PEMBUNGKUS DOMContentLoaded DIHAPUS UNTUK KOMPATIBILITAS BLOGGER

// =================================================================
// KONFIGURASI PENTING
// =================================================================
const GAS_URL =
  "https://script.google.com/macros/s/AKfycbyv0qyQ4rD2tvNoug7oeMefaU57zydf-uG0dn2djrKhAC6Z5AveE6d97Z3RXnmrWOU/exec";
...
const dom = {
  ...
  commentContentInput: document.querySelector(
    '#comment-form textarea[name="comment_content"]'
  ),
...
async function init() {
  console.log("Memulai Pemutar Musik v5.2 (Perbaikan Blogger)...");
  setupAuthObserver();
...
function setupAuthObserver() {
  fbAuth.onAuthStateChanged((user) => {
    if (user) {
      ...
      dom.userNameDisplay.textContent = `Halo, ${
...
function fetchSongInteractions(songId) {
  ...
  docRef.onSnapshot(
    ...
    (error) => {
      console.error("Gagal mengambil data interaksi: ", error);
    }
  );
}
...
async function postInteraction(songId, type) {
  ...
  await fbDb
    .runTransaction(async (transaction) => {
      ...
      let likes = doc.exists ? doc.data().likes || [] : [];
      let dislikes = doc.exists ? doc.data().dislikes || [] : [];

      if (type === "like") {
        if (likes.includes(userUid)) {
          transaction.update(docRef, {
            [likeField]: firebase.firestore.FieldValue.arrayRemove(userUid),
          });
        } else {
          transaction.set(
            docRef,
            { [likeField]: firebase.firestore.FieldValue.arrayUnion(userUid) },
            { merge: true }
          );
        ...
        if (dislikes.includes(userUid)) {
          transaction.update(docRef, {
            [dislikeField]: firebase.firestore.FieldValue.arrayRemove(userUid),
          });
        } else {
          transaction.set(
            docRef,
            {
              [dislikeField]: firebase.firestore.FieldValue.arrayUnion(userUid),
            },
            { merge: true }
          );
          if (likes.includes(userUid)) {
...
function switchToPlayerView(playlist, songIdToShow = null) {
  ...
  state.originalPlaylistOrder = playlist.songIds.map((id) =>
    musicDatabase.songs.find((s) => s.id === id)
  );
  state.currentPlaylist = state.isShuffle
    ? getShuffledPlaylist()
    : [...state.originalPlaylistOrder];
  ...
  if (songIdToShow) {
    const requestedIndex = state.originalPlaylistOrder.findIndex(
      (s) => s.id === songIdToShow
    );
  ...
  loadSong(
    initialSongIndex < state.originalPlaylistOrder.length ? initialSongIndex : 0
  );
  saveStateToStorage();
}
...
function renderPlaylists() {
  ...
  musicDatabase.playlists.forEach((playlist) => {
    ...
    card.innerHTML = `<img src="${playlist.thumbnail}" alt="${playlist.name}" onerror="this.src='https://placehold.co/400x400/333/ccc?text=Error'"><h3>${playlist.name}</h3><p>${playlist.description}</p>`;
...
function renderSonglist() {
  ...
  state.currentPlaylist.forEach((song) => {
    ...
    item.innerHTML = `<img src="${song.thumbnail}" alt="${song.title}" onerror="this.src='https://placehold.co/40x40/333/ccc?text=S'"><div class="b-player-song-item-info"><h4>${song.title}</h4><p>${song.artist}</p></div>`;
    item.addEventListener("click", () => {
      const originalIndex = state.originalPlaylistOrder.findIndex(
        (s) => s.id === song.id
      );
...
function updateInteractionDisplay(songId) {
  ...
  if (state.currentUser) {
    dom.likeBtn.classList.toggle(
      "active",
      data.likes.includes(state.currentUser.uid)
    );
    dom.dislikeBtn.classList.toggle(
      "active",
      data.dislikes.includes(state.currentUser.uid)
    );
  } else {
...
function renderComments(comments) {
  ...
  if (comments.length === 0) {
    dom.commentsContainer.innerHTML =
      '<p style="color: #555;">Jadilah yang pertama berkomentar!</p>';
    return;
  }
  const commentMap = {};
  const rootComments = [];
  ...
  rootComments.forEach((c) => {
    dom.commentsContainer.appendChild(createCommentElement(c));
    c.replies
      .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
      .forEach((r) =>
        dom.commentsContainer.appendChild(createCommentElement(r, true))
      );
  });
}
...
function createCommentElement(comment, isReply = false) {
  ...
  const nameHTML = comment.user_website
    ? `<a href="${comment.user_website}" target="_blank" rel="noopener nofollow">${comment.user_name}</a>`
    : comment.user_name;
  ...
  const processedContent = comment.comment_content
    ...
    .replace(
      /\[img\]\((.*?)\)/g,
      '<img src="$1" alt="Gambar dari user" style="max-width:100%; border-radius:8px; margin-top:10px;">'
    )
    .replace(
      /\[video\]\((.*?)\)/g,
      '<video src="$1" controls style="max-width:100%; border-radius:8px; margin-top:10px;"></video>'
    )
    .replace(
      /\|\|(.*?)\|\|/g,
  ...
  div.innerHTML = `
        <img class="b-player-comment-avatar" src="${
          comment.user_avatar || "https://placehold.co/40x40/555/ccc?text=U"
        }" onerror="this.src='https://placehold.co/40x40/555/ccc?text=U'">
        <div class="b-player-comment-body">
            <div class="b-player-comment-header"><span class="name">${nameHTML}</span><span class="time">${new Date(
    comment.timestamp
  ).toLocaleString("id-ID")}</span></div>
            <div class="b-player-comment-content">${processedContent}</div>
            <div class="b-player-comment-actions">
                <span class="action-btn reply-btn" data-comment-id="${
                  comment.comment_id
                }" data-user-name="${comment.user_name}">Balas</span>
                <span class="action-btn copy-comment-btn" data-comment-id="${
                  comment.comment_id
                }">Salin Link</span>
                <span class="action-btn report-comment-btn" data-comment-id="${
                  comment.comment_id
                }">Laporkan</span>
            </div>
        </div>`;
  ...
  div.querySelector(".copy-comment-btn").addEventListener("click", (e) => {
    const commentId = e.currentTarget.dataset.commentId;
    const songId = state.originalPlaylistOrder[state.currentSongIndex].id;
    const url = `${window.location.origin}${window.location.pathname}?playlist=${state.currentPlaylistId}&song=${songId}&comment=${commentId}`;
    navigator.clipboard
      .writeText(url)
      .then(() => showToast("Link komentar tersalin!"));
  });
  div.querySelector(".report-comment-btn").addEventListener("click", (e) => {
    const commentId = e.currentTarget.dataset.commentId;
    const reason = prompt("Apa alasan Anda melaporkan komentar ini?");
    if (reason) {
      const userEmail = prompt(
        "Masukkan email Anda (opsional) untuk kami hubungi kembali:"
      );
      apiPost("reportComment", {
...
function nextSong() {
  let nextIndex = state.isShuffle
    ? Math.floor(Math.random() * state.originalPlaylistOrder.length)
    : (state.currentSongIndex + 1) % state.originalPlaylistOrder.length;
  loadSong(nextIndex);
}
...
function prevSong() {
  let prevIndex =
    (state.currentSongIndex - 1 + state.originalPlaylistOrder.length) %
    state.originalPlaylistOrder.length;
  loadSong(prevIndex);
}
...
function getShuffledPlaylist() {
  ...
  let shuffled = [...state.originalPlaylistOrder].sort(
    () => Math.random() - 0.5
  );
  if (currentSong) {
...
// --- FUNGSI INTERAKSI DENGAN API (Google Sheets) ---
...
async function fetchComments(songId) {
  ...
  try {
    const response = await fetch(
      `${GAS_URL}?action=getComments&song_id=${songId}`
    );
    ...
    dom.commentsContainer.innerHTML =
      '<p style="color: #d9534f;">Gagal memuat komentar.</p>';
  }
}
...
async function postComment(e) {
  ...
  if (result && result.status === "sukses") {
    ...
    dom.parentCommentIdInput.value = "";
...
// --- FUNGSI PENYIMPANAN LOKAL & HELPER ---
...
function wrapTextInTextarea(before, after, placeholder = "") {
  const textarea = dom.commentContentInput;
  ...
  const text = `${textarea.value.substring(
    0,
    start
  )}${before}${newText}${after}${textarea.value.substring(end)}`;
  textarea.value = text;
  textarea.focus();
...
function setupEventListeners() {
  ...
  dom.copySongLinkBtn.addEventListener("click", () => {
    ...
    navigator.clipboard
      .writeText(url)
      .then(() => showToast("Link lagu tersalin!"));
  });
  ...
  dom.editorToolbar.addEventListener("click", (e) => {
    ...
    switch (action) {
      case "bold":
        wrapTextInTextarea("**", "**", "teks tebal");
        break;
      case "italic":
        wrapTextInTextarea("_", "_", "teks miring");
        break;
      case "link":
        wrapTextInTextarea("[link:", "](URL_ANDA)");
        break;
      case "spoiler":
        wrapTextInTextarea("||", "||", "spoiler");
        break;
      case "image":
        wrapTextInTextarea("[img]", "(URL_GAMBAR)");
        break;
      case "video":
        wrapTextInTextarea("[video]", "(URL_VIDEO)");
        break;
    }
  });
